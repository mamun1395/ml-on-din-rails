...
SAVED_MODEL_PATH = '../trt_saved_models/mlp_saved_model_TFTRT_FP32'
...
predictions = infer(tf.constant(X_test_float32))

key = next(iter(predictions))
prediction_classes = np.argmax(predictions[key], axis=1)
